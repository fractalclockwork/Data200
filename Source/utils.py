...
def read_files_from_zip(zip_file_path, regex_pattern):
    ...
    data_dict = (
        {}
    )  # Dictionary with name as key, and sub-dict of type and file as value
    ...
    with zipfile.ZipFile(zip_file_path, "r") as zip_ref:
        ...
        for file in all_files:
            ...
            if match:
                ...
                data_type = [
                    s for s in filtered if re.search(r"(test|train)", s)
                ][0]
        ...
        for name in tqdm(
            data_dict.keys(), ascii=False, desc=f"Importing data from zip"
        ):
            with zip_ref.open(data_dict[name]["file"]) as f:
                ...
                if match:
                    ...
                    # print('Reading Label:')
                    data_dict[name]["labels"] = np.load(
                        bytes_io, allow_pickle=True
                    )
...
def data2pd(image_dataset, label_dataset=None):
    ...
    for key in image_dataset.keys():
        ...
        if label_dataset != None:
            image_dataset[key]["labels"] = label_dataset[key][
                "labels"
            ]  # add labels
    ...
    if label_dataset != None:
        df = (
            df.T.explode(["images", "labels"])
            .drop(columns=["file"])
            .rename(columns={"labels": "label", "images": "image"})
        )
    else:
        df = (
            df.T.explode(["images"])
            .drop(columns=["file"])
            .rename(columns={"images": "image"})
        )
...
def save_model_data(name, model, history, notes=[], path="../Data"):
    ...
    with open(model_history_path, "wb") as db_file:
        ...
        pickle.dump(obj={"history": history, "notes": notes}, file=db_file)
